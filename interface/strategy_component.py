...
class StrategyEditor(tk.Frame):
    def __init__(self, root, binance: BinanceClient, *args, **kwargs):
        ...
        self._add_button = tkmac.Button(self._commands_frame, text="Add strategy",
                        font=GLOBAL_FONT, command=self._add_strategy_row, bg=BG_COLOR_2, 
                        fg=FG_COLOR, borderless=True)
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name][b_index] = tk.OptionMenu(self._table_frame, 
                                    self.body_widgets[code_name + "_var"][b_index], *base_param['values'])
    ...
    def _show_popup(self, b_index: int):
        ...
        for param in self._extra_params[strat_selected]:
            ...
            temp_label = tk.Label(self._popup_window, bg=BG_COLOR, fg=FG_COLOR, text=param['name'],
                                    font=BOLD_FONT)
        ...
        validation_button = tkmac.Button(self._popup_window, text="Validate", bg=BG_COLOR_2,
                                fg=FG_COLOR, command=lambda: self._validate_parameters(b_index), borderless=True)
